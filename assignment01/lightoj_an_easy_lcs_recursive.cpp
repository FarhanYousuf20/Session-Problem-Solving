...
ll LCS(ll n, ll m) {
        ...
        if(s[n - 1] == t[m - 1]) {
                ll tmp = 1 + LCS(n - 1, m - 1);
                ans[n][m] = ans[n - 1][m - 1] + s[n - 1];
                return dp[n][m] = tmp;
        } else {
                ll tmp1 = LCS(n - 1, m);
                ll tmp2 = LCS(n, m - 1);
                if(tmp1 > tmp2) ans[n][m] = ans[n - 1][m];
                else if(tmp1 < tmp2) ans[n][m] = ans[n][m - 1];
                else ans[n][m] = min(ans[n - 1][m], ans[n][m - 1]);
                return dp[n][m] = max(tmp1, tmp2);
        }
}
...
void solve() {
        ...
        ans = vector<vector<string>>(n + 1, vector<string>(m + 1));
